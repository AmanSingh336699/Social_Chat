...
const newGroupChat = TryCatch(async (req, res, next) => {
  ...
  const removeMember = TryCatch(async (req, res, next) => {
    ...
    if (chat.members.length <= 3)
      return next(new ApiError("Group must have at least 3 members", 400));
  ...
  const sendAttachments = TryCatch(async (req, res, next) => {
    ...
    if (files.length < 1)
      return next(new ApiError("Please Upload Attachments", 400));
  
    if (files.length > 5)
      return next(new ApiError("Files Can't be more than 5", 400));
    ...
    if (files.length < 1)
      return next(new ApiError("Please provide attachments", 400));
  ...
  const renameGroup = TryCatch(async (req, res, next) => {
    ...
    if (chat.creator.toString() !== req.user.toString())
      return next(
        new ApiError("You are not allowed to rename the group", 403)
      );
  ...
  const deleteChat = TryCatch(async (req, res, next) => {
    ...
    if (chat.groupChat && chat.creator.toString() !== req.user.toString())
      return next(
        new ApiError("You are not allowed to delete the group", 403)
      );
  
    if (!chat.groupChat && !chat.members.includes(req.user.toString())) {
      return next(
        new ApiError("You are not allowed to delete the chat", 403)
      );
    }
  ...
  const getMessages = TryCatch(async (req, res, next) => {
    ...
    if (!chat.members.includes(req.user.toString()))
      return next(
        new ApiError("You are not allowed to access this chat", 403)
      );
